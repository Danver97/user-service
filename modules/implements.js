...
function checkImplementation(interf, implementation) {
    ...
    Object.entries(implementation).forEach(([key, value]) => {
        if (!inter[key]) { return; }
        if (typeof value !== inter[key].type) { throw new Error(`The dbmanager implementation of ${key} don't satisfy the type of the property: required type ${inter[key].type}`); }
        if (typeof value === 'function' && value.length !== inter[key].args) { throw new Error(`The dbmanager implementation of ${key} don't satisfy the number of parameters: required ${inter[key].args}parameters.`); }
